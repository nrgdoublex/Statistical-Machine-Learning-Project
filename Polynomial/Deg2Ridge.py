...
plt.title('Training Error of Ridge Regression with Degree 2')
plt.xlabel('Number of iterations')
plt.ylabel('Root Mean Square Error')
plt.legend(loc="upper right")
plt.savefig("Deg2TrainRidgeRegre.png")

plt.figure()
for i in range(0,len(regularization_set)):
    plt.plot(np.arange(0,max_iteration),testing_error[i],label='Regularization Coeff=%0.3f' %regularization_set[i])
plt.axes().set_xscale('log')
plt.title('Testing Error of Ridge Regression with Degree 2')
plt.xlabel('Number of iterations')
plt.ylabel('Root Mean Square Error')
plt.legend(loc="upper right")
plt.savefig("Deg2TestRidgeRegre.png")